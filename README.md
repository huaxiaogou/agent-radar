...
- 中英文宽召回、正文补全后二次相关性过滤、URL 去重、eventKey 事件聚类和动态概念发现。详情正文失败时会保留 feed 摘要继续降级分析，但 item、来源和本轮任务都会显式标记为 degraded/partial，记录脱敏错误码，不会把 excerpt-only 冒充完整正文。高质量官方/实践者材料中的未知 AI Coding 工程术语不受固定概念枚举限制；每篇文章可输出 1–8 个独立概念，并在写入前以 `reuse-existing / create-new / needs-review` 身份裁决完成别名归一、复用或候选隔离。
...
RADAR_CONCEPT_BACKFILL_CONCURRENCY=2 npm run concepts:backfill
...
```bash
# 默认每批 20 篇、批内并发 2；命令会连续跑完全部批次并在成功后原子发布快照
RADAR_CONCEPT_BACKFILL_BATCH_SIZE=20 \
RADAR_CONCEPT_BACKFILL_CONCURRENCY=2 \
